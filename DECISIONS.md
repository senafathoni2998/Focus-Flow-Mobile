...
| `POST /habits/:id/checkin`, `POST /goals/:id/progress` | online-only | The WIRE is safe — both are key-wrapped. The **UI** is not: offline the tile does not move, the user taps again, and that is two ops with two DIFFERENT keys. An idempotency key defends against retry duplication, never against duplicate intent our own UI manufactured. Both overlays now exist; what is still missing is a "+2 pending" badge. Until then both refuse a `local_` target outright rather than POSTing to `/habits/local_…/checkin` — a 404 the user could do nothing about — and the habit card DISABLES its check-in control for an unsynced habit, because a button that can only fail is worse than one that is visibly unavailable. |
...
| `POST /goals/:id/progress` | online-only | STILL refused, and not for a wire reason — it is key-wrapped and safe to replay. Offline the card would not move, so the user taps again and produces two ops with two DIFFERENT keys. An idempotency key defends against retry duplication, never against duplicate intent our own UI manufactured. The overlay now exists; what is still missing is a "+2 pending" badge on the card. |
